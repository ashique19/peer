...
<h1><center>Blogcategories @if($blogcategories) : {{$blogcategories->total()}} @endif</center></h1>

<section class="col-sm-10 col-sm-offset-1">
    
    {!! Form::open(['class'=>'form form-inline', 'method' =>'post', 'url'=> action('Blogcategories@searchIndex')]) !!} 
...
<section class="col-sm-10 col-sm-offset-1">
    ...
    <a href="{{action('Blogcategories@create')}}" class="btn btn-default pull-right btn-lg blue-border blue-text">Add new</a>
    ...
    <table class="table table-responsive">
        <thead>
            <tr>
				...
				<th class="blue-bg white-text">Link</th>
            ...
            @if($blogcategories)
                @foreach($blogcategories as $blogcategory)
                    <tr>
						...
						<td><center><a href="{{$blogcategory->banner_photo}}" class="btn btn-default btn-xs"><img src="{{$blogcategory->banner_photo}}" width="100" height="70" ></a></center></td>
						<td>{{$blogcategory->created_at}}</td>
						<td>{{$blogcategory->updated_at}}</td>
                        <td>
                            {!! views('Blogcategories', $blogcategory->id, '<span class=\'fa fa-expand\'></span>', ['class'=>'btn btn-success btn-rounded']) !!}
                        </td>
                        <td>
                            {!! edits('Blogcategories', $blogcategory['id'], '<span class=\'fa fa-pencil\'></span>', ['class'=>'btn btn-warning btn-rounded']) !!}
                        </td>
                        <td>
