...
<section class="col-sm-10 col-sm-offset-1">
    ...
    {!! Form::open(['class'=>'form form-inline', 'method' =>'post', 'url'=> action('Blogs@searchIndex')]) !!} 
        ...
        <div class="form-group">
            ...
            {!! Form::select('status', [0=>'Draft', 1=>'Waiting for Review', 2=>'Published', 3=>'Trash'], old('status') , ['class'=>'form-control']) !!}
        </div>
    ...
    <table class="table table-responsive">
        <thead>
            <tr>
				...
				<th class="blue-bg white-text">popular</th>
				<th class="blue-bg white-text">Updated at</th>
                <th class="blue-bg white-text" width="50">Public</th>
            ...
            @if($blogs)
                @foreach($blogs as $blog)
                    <tr>
						...
						<td>@if($blog->is_popular == 1) Yes @elseif($blog->is_popular == 0) No @else $blog->is_popular @endif</td>
						<td>{{$blog->updated_at}}</td>
						<td><a href="{{action('BlogPublic@singleBlog' , $blog->link)}}" class="btn btn-primary btn-rounded"><i class="fa fa-pied-piper-alt"></i></a></td>
                        <td>
