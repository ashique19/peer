...
class MyProfile extends Controller
{
    ...
    public function edit()
    {
        ...
        if(auth()->user()->role == 3) return view('user.pages.my-profile');
    ...
    public function update(myProfileUpdateRequest $request)
    {
        ...
        $request['contact'] = $request->input('country_code') . $request->input('phone_no');
        // return $request->all();
        if( User::find(auth()->user()->id)->update( $request->all() ) )
        {
            
            if(auth()->user()->role == 3) return back()->withErrors('Profile updated successfully');
